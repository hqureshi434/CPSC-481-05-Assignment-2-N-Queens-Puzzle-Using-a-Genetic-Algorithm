...
def genetic_search(problem, ngen = 100000, pmut = 0.9, n = 20):
    ...
    states = [problem.randomState() for i in range(n)]
    best_fit = problem.N * (problem.N - 1) / 2
    gene_pool = [num for num in range(problem.N)]

    if problem.N == 1:
        return [0]
    elif problem.N == 2:
        return "2 queens will always conflict with each other"
    elif problem.N == 3:
        return "3 queens will always conflict with each other"

    return genetic_algorithm(states, problem.best_value, gene_pool = gene_pool, f_thres = best_fit, ngen = ngen)
...
def genetic_algorithm(population, fitness_fn, gene_pool=[0, 1], f_thres=None, ngen=1000, pmut=0.1):
    ...
    for i in range(ngen):
        ...
        fittes_individual = fitness_threshold(fitness_fn, f_thres, population)
        if fittes_individual:
            return fittes_individual

    print("Exceeded the generation count")
...
class NQueensProblem(Problem):
    ...
    def randomState(self):
        ...
        state = [random.randint(0, self.N-1) for x in range(self.N)]
    ...
    def h(self, node):
        ...
        num_conflicts = 0
        for(r1, c1) in enumerate(node.stae):
    ...
    def best_value(self, state):
        num_conflicts = 0
        max_conflicts = (len(state) * (len(state) - 1)) / 2
        for i in range(len(state) - 1):
            for j in range(i + 1, len(state)):
                if state[i] == state[j]:
                    num_conflicts += 1
                elif j - i == abs(state[j] - state[i]):
                    num_conflicts += 1
        return max_conflicts - num_conflicts
